...
bool FileWriter::writeFileCollection(const std::string& sstrFileName, const std::vector<std::string>& fileNames, const size_t numIdentities, const size_t numExpressions)
{
	const size_t dim = fileNames.size();
	...
	return FileWriter::writeFileCollection(sstrFileName, fileNames, permutation, numIdentities, numExpressions);
}

bool FileWriter::writeFileCollection(const std::string& sstrFileName, const std::vector<std::string>& fileNames, const std::vector<size_t>& permutation, const size_t numIdentities, const size_t numExpressions)
{
	...
	if(numFiles != permutation.size() || numFiles != numIdentities*numExpressions)
	{
		std::cout << "Wrong number of file names" << std::endl;
		return false;
	}

	outStream << numExpressions << " #Expressions" << std::endl;
	outStream << numIdentities << " #Identities" << std::endl;
	
	for(size_t i = 0; i < numFiles; ++i)
	{
		const size_t index = permutation[i];
		outStream << fileNames[index] << std::endl;
	}
