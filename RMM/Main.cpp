...
int main(int argc, char* argv[])
{
	...
	if(sstrIdentifier == "-tps" || sstrIdentifier == "tps")
	{
		...
		if(!MDLHelper::computeThinPlateSplines(sstrDataFolder + "/" + sstrFileCollectionName, sstrTextureCoordsFileName,  sstrDataFolder))
		{
	...
	else if(sstrIdentifier == "-rmm" || sstrIdentifier == "rmm")
	{
		//Robust multilinear model learning
		//1. Geometry and tps-files exist -> correspondence is optimized
		//2. Geometry exists AND no corrupt file exists -> missing data completion
		//3. Geometry exists AND corrupt file exists -> corrupt data reconstruction

		const std::string sstrDataFolder(argv[++index]);
		const std::string sstrFileCollectionName(argv[++index]);
		const std::string sstrModelLmkIndexFileName(argv[++index]);
		const std::string sstrCorruptFileCollectionName(argv[++index]);
		...
		MDLHelper::robustModelLearning(sstrDataFolder + "/" + sstrFileCollectionName, sstrDataFolder + "/" + sstrModelLmkIndexFileName,  sstrDataFolder + "/" + sstrCorruptFileCollectionName
											,  sstrDataFolder + "/" + sstrOuterBoundaryIndexFileName,  sstrDataFolder + "/" + sstrInnerBoundaryIndexFileName, sstrOutFolder);
	}
	else
	{
