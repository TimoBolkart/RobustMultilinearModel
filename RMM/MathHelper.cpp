...
const int NUM_ICP_ALIGNMENT_STEPS = 20;
...
bool MathHelper::computeICPAlignment(const std::vector<double>& source, const std::vector<double>& target, const std::vector<double>& targetNormals, const KDTree3& targetKDTree, const double maxIPPointDist
												, double& s, std::vector<double>& R, std::vector<double>& t, bool bScaling)
{
	bool bUsePointPlaneCorrespondence(!targetNormals.empty() && target.size() == targetNormals.size());

	const size_t numSourceVertices = source.size()/3;

	MathHelper::initTransformation(s, R, t);

	for(int iter = 0; iter < NUM_ICP_ALIGNMENT_STEPS; ++iter)
	{
		std::vector<double> trafoSource = source;
		MathHelper::transformData(s, R, "N", t, "+", trafoSource);

		const double currThreshold = std::pow(s*maxIPPointDist,2);

		std::vector<double> currSourceVertices;
		std::vector<double> currTargetVertices;

		for(size_t i = 0; i < numSourceVertices; ++i)
		{
			const size_t vertexOffset = 3*i;

			std::vector<double> currVertex;
			currVertex.push_back(trafoSource[vertexOffset]);
			currVertex.push_back(trafoSource[vertexOffset+1]);
			currVertex.push_back(trafoSource[vertexOffset+2]);

			int pointIndex(0);
			double sqrPointDist(0);
			targetKDTree.getNearestPoint(currVertex, pointIndex, sqrPointDist);

			if(sqrPointDist > currThreshold)
			{
				continue;
			}

			currSourceVertices.push_back(currVertex[0]);
			currSourceVertices.push_back(currVertex[1]);
			currSourceVertices.push_back(currVertex[2]);

			const size_t targetVertexOffset = 3*pointIndex;

			if(bUsePointPlaneCorrespondence)
			{
				//Use point-plane correspondence
				const Vec3d sourcePoint(currVertex[0], currVertex[1], currVertex[2]);
				const Vec3d nnPoint(target[targetVertexOffset], target[targetVertexOffset+1], target[targetVertexOffset+2]);
				const Vec3d nnNormal(targetNormals[targetVertexOffset], targetNormals[targetVertexOffset+1], targetNormals[targetVertexOffset+2]);

				Vec3d planeProjectionPoint;					
				MathHelper::getPlaneProjection(sourcePoint, nnPoint, nnNormal, planeProjectionPoint);

				currTargetVertices.push_back(planeProjectionPoint[0]);
				currTargetVertices.push_back(planeProjectionPoint[1]);
				currTargetVertices.push_back(planeProjectionPoint[2]);
			}
			else
			{
				//Use point-point correspondence
				currTargetVertices.push_back(target[targetVertexOffset]);
				currTargetVertices.push_back(target[targetVertexOffset+1]);
				currTargetVertices.push_back(target[targetVertexOffset+2]);
			}
		}
		...
		if(!MathHelper::computeAlignmentTrafo(currSourceVertices, currTargetVertices, tmp_s, tmp_R, tmp_t, bScaling))
		{
			std::cout << "Unable to compute rigid icp alignment" << std::endl;
			return false;
		}
...
bool MathHelper::computeCovarianceMatrix(const std::vector<double>& centeredData, const std::vector<size_t>& semCorr, const std::vector<size_t>& modeDim, const size_t mode, std::vector<double>& covarianceMatrix, size_t& matrixDim)
{
	if(mode != 2 && mode != 3)
	{
		std::cout << "MathHelper::computeCovarianceMatrix(...) - wrong mode input " << mode << std::endl;
		return false;
	}
	...
	const size_t numSampleVertices = d1/3;

	if(centeredData.size() != d1*d2*d3)
	{
		std::cout << "Wrong data dimension" << std::endl;
		return false;
	}

	if(semCorr.size() != d2*d3)
	{
		std::cout << "Wrong semantic correspondence dimension" << std::endl;
		return false;
	}
	...
	//Mode 2: 1/d3
	//Mode 3: 1/d2
	const double e3Reg = 1.0/static_cast<double>(e3);
...
#pragma omp parallel for
	for(int i = 0; i < e2; ++i)
	{
		//Mode 2: i
		//Mode 3: i*d2
		const size_t ix = i*x2_3;	

		for(size_t j = i; j < e2; ++j)
		{
			//Mode 2: j
			//Mode 3: j*d2
			const size_t jx = j*x2_3;
			...
			for(size_t r = 0; r < e3; ++r)
			{
				...
				//Mode 2: r*d2
				//Mode 3: r
				const size_t rx = r*x2_2;

				//Mode 2: iShape = i + r*d2
				//Mode 3: iShape = r + i*d2
				const size_t iShape = ix+rx;

				//Mode 2: jShape = j + r*d2
				//Mode 3: jShape = r + j*d2
				const size_t jShape = jx+rx;

				const size_t iShapeIndex = semCorr[iShape];
				const size_t jShapeIndex = semCorr[jShape];

				const size_t iShapeOffset = iShapeIndex*d1;
				const size_t jShapeOffset = jShapeIndex*d1;

				for(size_t k = 0; k < numSampleVertices; ++k)
				{
					//Mode 2
					...
					const size_t iVertexOffset = iShapeOffset + 3*k;
					const size_t jVertexOffset = jShapeOffset + 3*k;

					//Mode 2: c_ir[v_k] * c_jr[v_k]
					//Mode 3: c_ri[v_k] * c_rj[v_k]
					double tmpValue(0.0);
					tmpValue += centeredData[iVertexOffset]*centeredData[jVertexOffset];
					tmpValue += centeredData[iVertexOffset+1]*centeredData[jVertexOffset+1];
					tmpValue += centeredData[iVertexOffset+2]*centeredData[jVertexOffset+2];
