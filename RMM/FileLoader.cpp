...
bool FileLoader::readCorruptFileCollection(const std::string& sstrFileName, std::vector<std::string>& dataFileNames, std::vector<std::string>& corruptDataFileNames, std::vector<std::string>& corruptDataLmksFileNames)
{
	if(!fileExist(sstrFileName))
	{
		std::cout << "File does not exist " << sstrFileName << std::endl;
		return false;
	}
	...
	while(!bEnd)
	{
		...
		const std::string sstrFileName(strOutput);
		if(!sstrFileName.empty())
		{
			if(count%3 == 0)
			{
				dataFileNames.push_back(sstrFileName);
			}
			else if(count%3 == 1)
			{
				corruptDataFileNames.push_back(sstrFileName);
			}
			else if(count%3 == 2)
			{
				corruptDataLmksFileNames.push_back(sstrFileName);
			}
		}
	...
	return fclose(pFile)==0 && dataFileNames.size() == corruptDataFileNames.size() && dataFileNames.size() == corruptDataLmksFileNames.size();
}

bool FileLoader::loadIncompleteFileCollection(const std::string& sstrFileName, std::vector<double>& data, DataContainer& mesh, std::vector<std::string>& fileNames, std::vector<bool>& missingShapes, size_t& numExpressions, size_t& numIdentities)
{
	if(!readFileCollection(sstrFileName, fileNames, numExpressions, numIdentities))
	{
		std::cout << "Unable to read file collection" << std::endl;
		return false;
	}
	...
	for(; currFileNameIter != endFileNameIter; ++currFileNameIter)
	{
		...
		if(!FileLoader::fileExist(sstrCurrFileName))
		{
			continue;
		}
		...
		mesh = currMesh;
		numMeshVertices = currMesh.getNumVertices();
		break;
	}
	...
	missingShapes.clear();
	missingShapes.resize(numSamples, false);

	const size_t sampleDim = 3*numMeshVertices;
	data.resize(numSamples*sampleDim, 0.0);

#pragma omp parallel for
	for(int iFile = 0; iFile < numSamples; ++iFile)
	{
		//std::cout << iFile+1 << " of " << numSamples << std::endl;

		const std::string& sstrCurrName = fileNames[iFile];
		const std::string sstrCurrFileName = sstrFilePath + "\\" + sstrCurrName;
						
		if(!FileLoader::fileExist(sstrCurrFileName))
		{
			missingShapes[iFile] = true;
			continue;
		}
		...
		const size_t sampleOffset = iFile*sampleDim;
		for(size_t i = 0; i < sampleDim; ++i)
		{
			data[sampleOffset+i] = meshVertices[i];
		}
	}
...
bool FileLoader::loadCorruptFileCollection(const std::string& sstrFileName, std::vector<std::string>& dataFileNames, std::vector<DataContainer*>& corruptData, std::vector<std::vector<std::pair<double,bool>>>& corruptDataLmks)
{
	std::vector<std::string> corruptDataFileNames;
	std::vector<std::string> corruptDataLmksFileNames;
	if(!readCorruptFileCollection(sstrFileName, dataFileNames, corruptDataFileNames, corruptDataLmksFileNames))
	{
		std::cout << "Unable to read file collection" << std::endl;
		return false;
	}
	...
	const std::string sstrFilePath = getFilePath(sstrFileName);
	if(sstrFilePath.empty())
	{
		std::cout << "Unable to get file path" << std::endl; 
		return false;
	...
	const size_t numFiles = dataFileNames.size();
	corruptData.resize(numFiles, NULL);
	corruptDataLmks.resize(numFiles);

	for(size_t iFile = 0; iFile < numFiles; ++iFile)
	{		
		//Load corrupt geometry file
		const std::string sstrCorruptFile = sstrFilePath + "/" + corruptDataFileNames[iFile];

		DataContainer* pCurrMesh = new DataContainer();	
		if(!loader.loadFile(sstrCorruptFile, *pCurrMesh))
		{
			std::cout << "Unable to load corrupt data " << sstrCorruptFile << std::endl;

			delete pCurrMesh;

			for(size_t i = 0; i < corruptData.size(); ++i)
			{
				if(corruptData[i] != NULL)
					delete corruptData[i];
			}

			corruptData.clear();
			corruptDataLmks.clear();

			return false;
		}

		corruptData[iFile] = pCurrMesh;

		//Load landmarks of corrupt geometry file
		const std::string sstrCorruptLmkFile = sstrFilePath + "/" + corruptDataLmksFileNames[iFile];

		std::vector<double> lmks;
		std::vector<bool> lmksLoaded;
		if(loader.loadLandmarks(sstrCorruptLmkFile,lmks,lmksLoaded))
		{
			const size_t numLmks = lmksLoaded.size();
			
			std::vector<std::pair<double,bool>>& currLmks = corruptDataLmks[iFile];
			currLmks.resize(3*numLmks);

			for(size_t iLmk = 0; iLmk < numLmks; ++iLmk)
			{
				const size_t lmkOffset = 3*iLmk;
				currLmks[lmkOffset] = std::make_pair(lmks[lmkOffset], lmksLoaded[iLmk]);
				currLmks[lmkOffset+1] = std::make_pair(lmks[lmkOffset+1], lmksLoaded[iLmk]);
				currLmks[lmkOffset+2] = std::make_pair(lmks[lmkOffset+2], lmksLoaded[iLmk]);
			}
		}
		else
		{
			std::cout << "Unable to load corrupt data lmks " << sstrCorruptLmkFile << std::endl;
...
bool FileLoader::loadTpsFileCollection(const std::string& sstrFileName, std::vector<std::vector<double>>& vecCs, std::vector<std::vector<double>>& matAs, std::vector<std::vector<double>>& matWs, std::vector<std::vector<double>>& sourcePointsVec)
{
	...
	vecCs.resize(numShapes);
	matAs.resize(numShapes);
	matWs.resize(numShapes);
	sourcePointsVec.resize(numShapes);

	for(size_t iFile = 0; iFile < numShapes; ++iFile)
	{
		const std::string sstrCurrName = fileNames[iFile];
		const std::string sstrCurrGeometryFileName = sstrFilePath + "/" + sstrCurrName;
		if(!fileExist(sstrCurrGeometryFileName))
		{
			//Missing file
			continue;
		}

		const std::string sstrTpsFileName = sstrFilePath + "/" + FileLoader::getFileName(sstrCurrName) + ".tps";
		if(!fileExist(sstrTpsFileName))
		{
			std::cout << "No thin-plate spline for current file found " << sstrTpsFileName << std::endl;
			return false;
		}

		std::vector<double>& vecC = vecCs[iFile];
		std::vector<double>& matA = matAs[iFile]; 
		std::vector<double>& matW = matWs[iFile]; 
		std::vector<double>& sourcePoints = sourcePointsVec[iFile];
		if(!loadThinPlateSpline(sstrTpsFileName, vecC, matA, matW, sourcePoints))
		{
			return false;
		}
	}
